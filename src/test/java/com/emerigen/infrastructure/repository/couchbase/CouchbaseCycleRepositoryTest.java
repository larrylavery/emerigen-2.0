...
public class CouchbaseCycleRepositoryTest {
	...
	public final void givenSameCycleLoggedTwice_WhenCountQueriedByKey_ThenCountShouldBe1() {
		...
		JsonObject cycleNodeJsonDoc = JsonObject.create().put("sensorType", Sensor.TYPE_HEART_RATE)
				...
				.put("minimumDelayBetweenReadings", 1000).put("reportingMode", "1").put("wakeUpSensor", false)
		...
		CouchbaseRepository.getInstance().log("cycle",
				"" + Sensor.TYPE_HEART_RATE + Sensor.LOCATION_WATCH + "Daily", cycleJsonDoc);
		CouchbaseRepository.getInstance().log("cycle",
				"" + Sensor.TYPE_HEART_RATE + Sensor.LOCATION_WATCH + "Daily", cycleJsonDoc);
		...
		N1qlQueryResult result = CouchbaseRepository.getInstance().query("cycle",
				N1qlQuery.parameterized("SELECT COUNT(*) FROM `cycle` WHERE sensorType = $sensorType"
