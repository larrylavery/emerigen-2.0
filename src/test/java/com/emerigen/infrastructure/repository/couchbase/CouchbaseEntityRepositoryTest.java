...
//	    }
		...
		//Given - an Entity logged with two channelTypes pointing to a local sensoryEvent file
		SoftAssertions softly = new SoftAssertions();
		JsonObject jsonObjectChannel  = JsonObject.create()
				.put("channelType", "channelType1")
				.put("sensoryEventsUri", "/Information/dev/logger/src/main/resources/channel-type-1.csv")
		...
		JsonObject entityJsonDoc = JsonObject.create()
				.put("entityID", entityUuid1)
				.put("timestamp", "1")
		...
		try {
			Thread.sleep(500 + Long.parseLong(EmerigenProperties.getInstance().getValue("couchbase.server.logging.catchup.timer")));
		...
		N1qlQueryResult result = CouchbaseRepository.getInstance().query("entity",N1qlQuery.simple(queryString));
		...
		JsonObject entityJsonObject = ((N1qlQueryRow) result.allRows().get(0)).value().getObject("entity");
		...
		softly.assertThat(entityJsonObject.getArray("channels").equals(JsonArray.from(jsonObjectChannel, jsonObjectChannel2)));
		softly.assertAll();
		...
		//And Then
		//Verify that learning-complete is marked on one channel (and not the other) when the last sensoryEvent is consumed
	}
	...
	public final void givenEntityWithOneChannel_WhenChannelQueried_ThenItsSensoryEventSourceShouldBeValid() {
		...
		String entityUuid1 = UUID.randomUUID().toString(); 
		JsonObject channelType = JsonObject.create()
				.put("channelType", "channelType1")
				.put("sensoryEventsURI", "/Information/dev/logger/src/main/resources/channel-type-1.csv")
				.put("learningComplete", true);
		...
		JsonObject entityJsonDoc = JsonObject.create()
				.put("entityID", entityUuid1)
				.put("timestamp", "1")
				.put("channels", JsonArray.from(channelType));
		
		CouchbaseRepository.getInstance().log("entity", entityUuid1, entityJsonDoc);
		
		//Give the bucket a chance to catch up after the log
		try {
			Thread.sleep(Long.parseLong(EmerigenProperties.getInstance().getValue("couchbase.server.logging.catchup.timer")));
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		JsonObject entity2JsonObject = ((N1qlQueryRow) result.allRows().get(0)).value().getObject("entity");
		...
		softly.assertThat(entity2JsonObject.getArray("channels")).isEqualTo(
				JsonArray.from(channelType));
